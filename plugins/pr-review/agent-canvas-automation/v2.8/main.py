"""GitHub PR Reviewer — OpenHands Automation (v2.8 — duplicate-review guard).
...
# REPO is the deployment-specific target. It must be filled in before this
# script runs; the `/pr-reviewer:setup` skill does this for you in its
# "Apply the deployment constants" step (look for the `REPO = ...` line).
# Until REPO is set, `_verify_token_and_repo` will fail with a 404 against
# an empty path, which is the desired fail-fast behaviour.
REPO = ""
...
def _github_request(token: str, method: str, path: str, body: dict | None = None) -> tuple[dict, dict]:
    ...
    headers = {
        ...
        "User-Agent": "openhands-pr-reviewer-automation/2.8",
    }
...
def _existing_bot_review_for_commit(
    reviews: list[dict], bot_login: str, commit_id: str
) -> dict | None:
    """Return the bot's review on the given commit, or None.

    Matches by exact `commit_id` when possible (cleanest check), but also
    returns the most recent bot review at the PR head if no exact match
    exists — that handles the case where the head has advanced since the
    agent posted.
    """
    bot = bot_login.lower()
    by_commit = [
        r for r in reviews
        if (r.get("user") or {}).get("login", "").lower() == bot
        and r.get("commit_id", "") == commit_id
    ]
    if by_commit:
        return sorted(by_commit, key=lambda r: r.get("submitted_at", ""), reverse=True)[0]
    bot_only = [
        r for r in reviews
        if (r.get("user") or {}).get("login", "").lower() == bot
    ]
...
def _check_conversation_completion(
    ...
    if status in {"error", "stuck"}:
        ...
        # v2.8: Always check for existing bot review at the same commit
        ...
        if payload is None:
            # No JSON contract from the agent. If the bot already posted
            # (via MCP), close the state without re-posting.
            ...
            # v2.8: even with a parseable payload, skip if the bot already
            # posted a review (e.g. via the GitHub MCP). Posting again would
